package com.example.administrator.cartspclient.rtsp.clinet.Socket;

import android.util.Log;


import com.example.administrator.cartspclient.rtsp.clinet.Stream.RtpStream;
    ...
    private void analysisOnePacket(byte[] data, int offset){
        ...
        while(tmp[1] != -1) {
            ...
            if(tmp[1] < datalen) {
                ...
                buffer = new byte[tmp[1]];
                System.arraycopy(data,tmp[0], buffer, 0, tmp[1]);
                RtpStream.receiveData(buffer,buffer.length);
                rtspBuffer.inNextPacket = false;
                break;
            }
        }
    ...
    private int[] getRtspFrameInfo(byte[] data,int offset){
        ...
        if(haveRtspFrame) {
            if(mOffset + 3 < data.length) {
                ...
                length = -2; //This time 0x24 0x00 and data length is not in one packet
            }
        }
        else{
            length = -1;
