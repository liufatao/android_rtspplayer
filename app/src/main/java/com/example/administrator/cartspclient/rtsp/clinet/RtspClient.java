package com.example.administrator.cartspclient.rtsp.clinet;
...
import com.example.administrator.cartspclient.rtsp.clinet.Socket.RtpSocket;
import com.example.administrator.cartspclient.rtsp.clinet.Video.H264Stream;
...
public class RtspClient {
    ...
    private void sendRequestSetup() throws IOException {
        ...
        matcher = Response.regexSessionWithTimeout.matcher(Response.headers.get("session"));
        if (matcher.find()) {
            mSession = matcher.group(1);
        } else {
            mSession = Response.headers.get("session");
        }
        Log.d(tag, "the session is " + mSession);

        //get the port information and start the RTP socket, ready to receive data
        if (isTCPtranslate) {
            matcher = Response.regexTCPTransport.matcher(Response.headers.get("transport"));
        } else {
            matcher = Response.regexUDPTransport.matcher(Response.headers.get("transport"));
        }
        if (matcher.find()) {
            ...
            while ((line = input.readLine()) != null) {
                if (line.length() > 3 || Describeflag) {
                    ...
                    if (Describeflag) {
                        ...
                        if (matcher.find()) {
                            if (foundMediaType == 1) {
                                sdpInfo.audioTrack = matcher.group(1);
                            } else if (foundMediaType == 2) {
                                sdpInfo.videoTrack = matcher.group(1);
                            }
                        }
                        ...
                        if (sdpStartFlag) {
                            sdpHaveReadLength += line.getBytes().length + 2;
                        }
